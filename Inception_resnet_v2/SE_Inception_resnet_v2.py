...
def writeToCsv(test_name,predict_label,file_path=TEST_SAVE_PATH):
    ...
    file_path1 = file_path + 'test_9_'+'_epoch_2'+'_67'+'.csv'
    try:
...
def data_label_load(data_name,data_label,data_type = 'train'):
    ...
    pic_data = np.zeros([len(X_name), image_wight,image_hight,3])#(-1,299,150,3)
    for img_index in range(len(X_name)):
        img_dir = os.path.join(data_path, X_name[img_index])
        img = Image.open(img_dir)#(120*299*3)
        image_array = np.array(img).transpose(1,0,2)#(299*150*3)
...
def Evaluate(sess):
    ...
    for it in range(test_iteration):
        ...
        pre_labels_ = labels_max_idx.eval(feed_dict=test_feed_dict)
        writeToCsv(test_batch_name, pre_labels_)
...
class SE_Inception_resnet_v2():
    ...
    def Build_SEnet(self, input_x):
        input_x = tf.pad(input_x, [[0, 0], [0, 0], [50, 50], [0, 0]])
        # size 200*100 -> 200*200
        print(np.shape(input_x))
        # only baidu architecture
...
train_name, train_label, test_name, test_label = prepare_data()

# image_size = 200*100, img_channels = 3, class_num = 100
...
with tf.Session() as sess:
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for step in range(1, iteration + 1):
            # 这里batch循环读取
            if pre_index + batch_size < 57173:  # 57173
